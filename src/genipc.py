...
#FIXME this file is too long, should be split up into a class or two among
#different files
...
class TypedElement:
	def __init__(self, xml_element):
		...
		# FIXME: We're only looking at the top level tag here,
		#        but we should handle nested types as well.
		self.type = choices[0].nodeName
...
class genipc:
    def __init__(self):
	self.start_processmsg()
	self.start_deser()

	self.start_processreq()
	self.start_reqdeser()
	...
	self.do_enums(enum_list,nodes)

	#parse all objects
	print "Parsing objects"
	object_list = nodes[0].getElementsByTagName("object")
	self.do_objects(object_list,nodes)
    ...
    def write_deserialization(self,file,args,req = False):
	if not req:
	    if args.length > 0:
		for arg in args:
		    #FIXME lists of complex types, complex types
    ...
    def output_deserialize_client(self,obj,method):
	#return type
	...
	self.write_deserialization(ipcreqdeser,retvalstr,True)

	ipcreqdeser.write("}\n\n")

    #write the deserialize function for a specific ipc method(server)
    def output_deserialize_server(self,obj,method):
	#return type
	retvalstr = get_retval(method);
	...
	self.write_deserialization(ipcmsgdeser,args)
    ...
    def output_deserialize_server_prop(self,obj,prop):
	#return type
    ...
    *msg)\n{\n")
    ...
    def write_processmsg(self,node, req = False):
	if not req:
	    outfile = ipcmsggen;
	else:
	    outfile = ipcreqgen;
    ...
    retmsg);\n")
	    ...
	    if not req:
		    self.output_deserialize_server(node,method)
	    elif req:
		    if retvalstr != "none":
    ...
    #				method.getAttribute("name"))
		    ...
		    self.output_deserialize_client(node,method)
    ...
    xmms_ipc_msg_get_cookie (msg));\n")
		...
		self.output_deserialize_server_prop(node,prop)
    ...
    def write_cmd_code(self,node):
	cfile.write("/* generated by genipc.py */\n\n")
    ...
    def do_objects(self,objects,nodes):
	for node in objects:
		...
		node.getAttribute("type") == "both":
		    ...
		    self.write_processmsg(node)
		    self.write_processmsg(node,True)
...
# imports the given python script
def myimp(filename):
	fp = file(filename, 'r')
	description = ('.py', 'r', imp.PY_SOURCE)

	try:
		return imp.load_module(filename, fp, filename, description)
	finally:
		fp.close()
...
if __name__ == "__main__":
	ipc = parse_xml(sys.argv[1])

	builder = myimp(sys.argv[2])
	builder.build(ipc)
