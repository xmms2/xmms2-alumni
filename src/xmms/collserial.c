...
static void write_coll_attributes (const char *key, const char *value, void *udata);
...
xmms_collection_dag_save (xmms_coll_dag_t *dag)
{
	...
	for (i = 0; i < XMMS_COLLECTION_NUM_NAMESPACES; ++i) {
		...
		path = XMMS_BUILD_PATH (COLL_DIR, namespace);
...
xmms_collection_write_operator (xmmsv_coll_t *coll, FILE *file)
{
	...
	/* Write attributes */
	xmmsv_coll_attribute_foreach (coll, write_coll_attributes, file);
...
write_operator (void *key, void *value, void *udata)
{
	...
	char *path = XMMS_BUILD_PATH (COLL_DIR, namespace, label);
...
static void
write_coll_attributes (const char *key, const char *value, void *udata)
{
	FILE *file = udata;
