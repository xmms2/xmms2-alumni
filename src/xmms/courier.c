...
static GList *xmms_courier_client_get_connected_clients (xmms_courier_t *courier, xmms_error_t *err);
...
xmms_courier_init (void)
{
	...
	courier->clients_lock = g_mutex_new ();

	xmms_courier_register_ipc_commands (XMMS_OBJECT (courier));

	xmms_object_connect (XMMS_OBJECT (xmms_ipc_manager_get()),
...
xmms_courier_destroy (xmms_object_t *object)
{
	...
	g_mutex_free (courier->clients_lock);
                                           ...
                                           xmms_error_t *err)
{
	...
	for (next = courier->clients; next; next = g_list_next (next)) {
		val = xmmsv_new_int (GPOINTER_TO_INT (next->data));
		ret = g_list_prepend (ret, (gpointer) val);
	}
