...
xmms_browse_list_sortfunc (gconstpointer a, gconstpointer b)
{
	...
	g_return_val_if_fail (val1->type == XMMS_VALUE_TYPE_DICT, 0);
	g_return_val_if_fail (val2->type == XMMS_VALUE_TYPE_DICT, 0);

	tmp1 = g_tree_lookup (val1->value.dict, "intsort");
	tmp2 = g_tree_lookup (val2->value.dict, "intsort");

	if (tmp1 && tmp2) {
		g_return_val_if_fail (tmp1->type == XMMS_VALUE_TYPE_INT32, 0);
		g_return_val_if_fail (tmp2->type == XMMS_VALUE_TYPE_INT32, 0);
	...
	g_return_val_if_fail (tmp1->type == XMMS_VALUE_TYPE_STRING, 0);
	g_return_val_if_fail (tmp2->type == XMMS_VALUE_TYPE_STRING, 0);
                             ...
                             gint32 *val)
{
	...
	if (obj && obj->type == XMMS_VALUE_TYPE_INT32) {
		*val = obj->value.int32;
		ret = TRUE;
	}
                             ...
                             const gchar **val)
{
	...
	obj = xmms_xform_metadata_get_val (xform, key);
	if (obj && obj->type == XMMS_VALUE_TYPE_STRING) {
...
add_metadatum (gpointer _key, gpointer _value, gpointer user_data)
{
	...
	if (value->type == XMMS_VALUE_TYPE_STRING) {
		xmms_medialib_entry_property_set_str_source (st->session,
		                                             st->entry,
		                                             key,
		                                             value->value.string,
		                                             st->source);
	} else if (value->type == XMMS_VALUE_TYPE_INT32) {
                            ...
                            gpointer *data, gssize *datalen)
{
	...
	obj = xmms_xform_auxdata_get_val (xform, key);
	if (obj && obj->type == XMMS_VALUE_TYPE_BIN) {
