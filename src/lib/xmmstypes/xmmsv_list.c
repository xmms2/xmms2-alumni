...
static int _xmmsv_list_resize (xmmsv_list_internal_t *l, int newsize);
static int _xmmsv_list_insert (xmmsv_list_internal_t *l, int pos, xmmsv_t *val);
static int _xmmsv_list_append (xmmsv_list_internal_t *l, xmmsv_t *val);
static int _xmmsv_list_remove (xmmsv_list_internal_t *l, int pos);
static int _xmmsv_list_move (xmmsv_list_internal_t *l, int old_pos, int new_pos);
static void _xmmsv_list_clear (xmmsv_list_internal_t *l);
...
/**
 * Move the element from position @c old_pos to position @c new_pos.
 *
 * The #xmmsv_list_iter_t's remain pointing at their element (which might
 * or might not be at a different position).
 *
 * @param listv A #xmmsv_t containing a list
 * @param old_pos The original position in the list. If negative, start counting
 *                from the end (-1 is the last element, etc.)
 * @param new_pos The new position in the list. If negative start counting from the
 ...
 *
 * @param listv A #xmmsv_t containing a list.
 * @param comparator The comparison function must return an integer less than,
