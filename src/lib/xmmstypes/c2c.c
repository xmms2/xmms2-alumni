...
{ \
	ctype ret; \
\
	x_return_val_if_fail (getter (msg, #field, &ret), failval); \
	return ret; \
}
...
GEN_C2C_FIELD_GET_FUNC (id, xmmsv_dict_entry_get_int, int, -1)
...
GEN_C2C_FIELD_GET_FUNC (sender, xmmsv_dict_entry_get_int, int, -1)
...
GEN_C2C_FIELD_GET_FUNC (destination, xmmsv_dict_entry_get_int, int, -1)
 ...
 */
xmmsv_t *
xmmsv_c2c_message_format (int32_t sender, int32_t dest, int32_t id,
                          xmmsv_t *payload)
{
	xmmsv_ref (payload);
