 ...
 */
bool
xmmsc_sc_setup (xmmsc_connection_t *c)
{
	xmmsc_result_t *res;
...
on_message_received (xmmsv_t *c2c_msg, void *userdata)
{
	...
	if (!xmmsv_dict_entry_get_int (payload, XMMSC_SC_CMD_KEY, &cmd)) {
...
command_call (xmmsc_connection_t *c, xmmsv_t *call)
{
	...
	method = locate_interface_entity (c, method_path);
	if (!method) {
...
command_broadcast_subscribe (xmmsc_connection_t *c, xmmsv_t *path, int msgid)
{
	...
	broadcast = locate_interface_entity (c, path);
	if (!broadcast) {
...
command_introspect (xmmsc_connection_t *c, xmmsv_t *args)
{
	...
	ifent = locate_interface_entity (c, path);
	if (!ifent) {
...
locate_interface_entity (xmmsc_connection_t *c, xmmsv_t *path)
{
	...
	return xmmsc_sc_namespace_resolve_path (root, path);
}

void
create_root_namespace (xmmsc_connection_t *c)
{
	xmmsv_t *clientname;
