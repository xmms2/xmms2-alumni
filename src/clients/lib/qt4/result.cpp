...
#include <xmmsclient/xmmsclientqt4/playback.h>
...
namespace XMMSQt
{
	...
	ResultBase::ResultBase (Client *client, int cookie) : QObject ()
	{
		m_cookie = cookie;
		m_object = NULL;
		m_slot = NULL;
	...
	ResultBase::ResultBase (QString apierr) : QObject  ()
	{
	...
	void
	ResultBase::exec (const Message &msg)
	{
		m_message = msg;
...
//					                           Q_ARG (Playback::Status, (Playback::Status) m_message.getUInt32 ()));
	...
	void
	ResultBase::operator () (QObject *object, const char *slot)
	{
		setSlots (object, slot, NULL, NULL);
	}

	void
	ResultBase::operator () (QObject *object, const char *slot, const char *errslot)
	{
		setSlots (object, slot, NULL, errslot);
	}

	void
	ResultBase::operator () (QObject *object, const char *slot, QObject *errobject,
	                    const char *errslot)
	{
	...
	void
	ResultBase::setSlots (QObject *object, const char *slot, QObject *errobject,
	                  const char *errslot)
	{
		m_object = object;
		m_slot = slot;
...
#include "xmmsclient/xmmsclientqt4/result.moc"
