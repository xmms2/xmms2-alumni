...
module Xmms::Client
	...
	class Client
		...
		attr_reader :name, :socket,
		            :playlist, :config, :output, :medialib
		...
		def io_in_handle
			...
			done = @read_message.read_cb @socket
		...
		def process_message(message)
			...
			found = @results[index]
			found.process_reply message

			# FIXME: only delete if the callback cannot hit again
