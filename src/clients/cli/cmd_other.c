...
cmd_plugin_load (xmmsc_connection_t *conn, gint argc, gchar **argv)
{
	...
	if (argc < 3) {
		print_error ("Need to specify module to load");
	}
...
cmd_plugin_unload (xmmsc_connection_t *conn, gint argc, gchar **argv)
{
	...
	if (argc < 3) {
		print_error ("Need to specify module to unload");
	}
...
cmd_plugin_reload (xmmsc_connection_t *conn, gint argc, gchar **argv)
{
	...
	if (argc < 3) {
		print_error ("Need to specify module to reload");
	}
	...
	print_info ("module reloaded from %s", name);
	xmmsc_result_unref (res);
}
