...
def emit_method_define_code(object, method, c_type):
	...
	crap = [c_map[a.type[0]] for a in method.arguments]

	while len(crap) < 6:
		crap.append('NONE')

	crap2 = ', ' + ', '.join(x for x in crap)

	Indenter.printline('XMMS_CMD_DEFINE6 (%s, %s, %s, %s%s);' % (method.name, full_method_name, c_type, return_value_type, crap2))
